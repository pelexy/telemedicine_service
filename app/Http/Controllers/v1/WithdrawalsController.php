  ...
  /**
    ...
    *     @OA\Parameter(
    ...
    public function getwithdrawalsbyuserid($id){
        $withdrawals = $this->withdrawal->querable()->where('user_id', $id)->with('user')->get();
        return response()->json($withdrawals, 200);
    }
    ...
    *     @OA\Parameter(
    ...
    public function confirmation(){
            ...
            $withdraw = $this->withdrawal->querable()->where('transfer_code', $event->transfer_code);
            if($event->event == "transfer.success"){
                $withdraw->update(['status' => 'success']);
                $transaction_credit = $this->transaction->add([
                    "reference_id" => $event->transfer_code,
                    "isdebit"   => 0,
